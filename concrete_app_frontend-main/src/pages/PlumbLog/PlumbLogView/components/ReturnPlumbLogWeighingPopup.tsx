...
import { putIsReturnChangeIndependentWeighingData } from "store/features/apiSlice"
...
export const ReturnPlumbLogWeighingPopup = ({ requestID, popup, setPopup, refreshData }: props) => {
    ...
    const returnHandler = async () => {
        const obj: any = {
            id: requestID,
            is_return: true,
            return_note: description
        }
        const response = await dispatch(putIsReturnChangeIndependentWeighingData(obj))
                ...
                <div className="df fdc w30" style={{ background: 'white', borderRadius: '32px', padding: '20px', gap: '15px'}}>
                    <div className="df fdc" style={{gap: '5px'}}>
                        <div className="df fdr jcsb aic">
                            <span className="fw700 fz20" style={{ marginRight: '30px' }}>Подтвердите возврат</span>
