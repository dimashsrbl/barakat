...
import { getMaterialsData } from 'store/features/apiSlice'
...
export const Materials = () => {
    ...
    const getMaterials = useCallback(async () => {
        ...
        const response = await dispatch(getMaterialsData({limit: !isMobile ? pageSize : null, is_active: isActive, name: searchValue, offset: newOffset}));
                ...
                <div className={`${s.dataMobileBlock} df fdc`}
                    ...
                    <div className='df fdc' style={{gap: '6px'}}>
                        ...
                        <div className="df fdr jcsb">
                            <span className='fz16 cg'>{item?.material_type?.name || '-'}</span>
                            <span className='fz16 cg'>{item?.density || '-'}</span>
                        </div>
                    </div>
                </div>
            ))}
    ...
    const RenderDesktopComponent = () => (
        ...
        <div className={`${s.contentBlock} df fdc`}>
            <div className={`${s.row} df jcsb aic fz18 fw500`}>
              <span className='w50'>Наименование</span>
              <span className='w20'>Тип</span>
              <span className='w30'>Удельный вес</span>
            </div>
            ...
            {data && data.map((item:any, index:any) => (
                ...
                key={index}
                >
                    <span className='w50'>{item?.name || '-'}</span>
                    <span className='w20'>{item?.material_type?.name}</span>
                    <span className='w30'>{item?.density || '-'}</span>
                </div>
            ))}
    ...
    const handlePageChange = async (pageNumber: any) => {
        if (pageNumber >= 1 && pageNumber <= totalPages) {
            ...
            const response = await dispatch(getMaterialsData({
                limit: pageSize, 
                offset: newOffset, 
                is_active: isActive,
                name: searchValue,
