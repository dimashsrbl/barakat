...
import { getDriversData } from 'store/features/apiSlice'
...
export const Drivers = () => {
    ...
    const getDrivers = useCallback(async () => {
        ...
        const response = await dispatch(getDriversData({limit: !isMobile ? pageSize : null, is_active: isActive, name: searchValue, offset: newOffset}));
    ...
    const RenderDesktopComponent = () => (
        ...
        <div className={`${s.contentBlock} df fdc`}>
            ...
            {data && data.map((item:any, index:any) => (
                ...
                key={index}
                >
                    <span className='w100'>{item?.name || '-'}</span>
                </div>
            ))}
    ...
    const handlePageChange = async (pageNumber: any) => {
        if (pageNumber >= 1 && pageNumber <= totalPages) {
            ...
            const response = await dispatch(getDriversData({
                limit: pageSize, 
                offset: newOffset, 
                is_active: isActive,
                name: searchValue,
