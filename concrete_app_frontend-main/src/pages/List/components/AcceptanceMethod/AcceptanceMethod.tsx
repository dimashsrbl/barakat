...
import { getAcceptanceMethodData } from 'store/features/apiSlice';
...
export const AcceptanceMethod = () => {
    ...
    const getAcceptanceMethod = useCallback(async () => {
        ...
        const response = await dispatch(getAcceptanceMethodData(
            {
            ...
            {data && data.map((item:any, index:any) => (
                <div className={`${s.dataMobileBlock} df fdc`}
                    ...
                    <div className='df fdc' style={{gap: '6px'}}>
                        ...
                        <span className='fz16 cg'>{item?.receive_method_type?.name || '-'}</span>
                    </div>
        ...
        <div className={`${s.contentBlock} df fdc`}>
            ...
            {data && data.map((item:any, index:any) => (
                ...
                key={index}>
                    ...
                    <span className='w30'>{item?.receive_method_type?.name || '-'}</span>
