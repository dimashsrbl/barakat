...
import { getCarrierData } from 'store/features/apiSlice'
...
export const Carrier = () => {
    ...
    const getCarrier = useCallback(async () => {
        ...
        const response = await dispatch(getCarrierData({limit: !isMobile ? pageSize : null, is_active: isActive, name: searchValue, offset: newOffset}));
    ...
    const RenderDesktopComponent = () => (
        ...
        <div className={`${s.contentBlock} df fdc`}>
            <div className={`${s.row} df jcsb aic fz18 fw500`}>
              ...
              <span className='w50'>Примечание</span>
            </div>
            ...
            {data && data.map((item:any, index:any) => (
                ...
                key={index}>
                    ...
                    <span className='w50'>{item?.description || '-'}</span>
                </div>
            ))}
    ...
    const handlePageChange = async (pageNumber: any) => {
        if (pageNumber >= 1 && pageNumber <= totalPages) {
            ...
            const response = await dispatch(getCarrierData({
                limit: pageSize, 
                offset: newOffset, 
                is_active: isActive,
                name: searchValue,
